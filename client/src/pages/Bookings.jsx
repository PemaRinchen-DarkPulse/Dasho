...
import { BookingAPI, getToken } from "@/lib/api";
...
const Bookings = () => {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError("");
      try {
        // Fetch only confirmed bookings from server
        const data = await BookingAPI.listConfirmed();
        if (!mounted) return;
        // Map to UI shape and compute duration minutes
        const mapped = (data || []).map((b) => ({
          ...
          endTime: b.endTime,
          // compute duration minutes from HH:mm difference
          durationMinutes: (() => {
            try {
      ...
      <div className="container mx-auto px-4 py-8">
              ...
              <div className="overflow-x-auto rounded-lg border">
                ...
                <tbody>
                  {filteredBookings.map((b, idx) => (
                    <tr 
                      key={b.id} 
                      className="border-t odd:bg-white even:bg-muted/20 hover:bg-accent/20 transition-colors"
                    >
                      <td className="px-4 py-3 font-medium">{idx + 1}</td>
                      <td className="px-4 py-3">
                        <Link to={`/equipment/${b.equipmentId}`} className="text-primary hover:underline">
                          {b.equipmentName}
                        </Link>
                      </td>
                      <td className="px-4 py-3">
        ...
        {filteredBookings.length === 0 && !loading && (
          <div className="text-center py-12 min-h-[50vh] flex flex-col items-center justify-center">
            ...
            {/* Intentionally no CTA on public bookings page empty state */}
          </div>
        )}
            ...
            <Card className="shadow-card">
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-green-600">
                  {new Set(bookings.map(b => b.equipmentId)).size}
                </div>
            ...
            <Card className="shadow-card">
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-gray-600">
                  {Array.from(new Set(bookings.map(b => b.date))).length}
                </div>
